...
def merge_json_namespaces(output_file, schema_file):
    ...
    try:
        ...
        for game in sorted(os.listdir(input_dir)):
            game_path = os.path.join(input_dir, game)
            if not os.path.isdir(game_path):
                continue

            for apiset in sorted(os.listdir(game_path)):
                apiset_path = os.path.join(game_path, apiset)
                if not os.path.isdir(apiset_path):
                    continue

                for namespace in sorted(os.listdir(apiset_path)):
                    namespace_path = os.path.join(apiset_path, namespace)
